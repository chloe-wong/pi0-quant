"""vector_engine_model.py — top-level dispatcher.
...
FP8 lane_boxes), and returns the final BF16 lane vector. It is the
reference used by `gen_vectors.py` to compute golden values for the
`vpu_vectors.txt` cases and by the Phase 3 torch adapter.

This is the **functional** entry point only. `cycle_step()` remains a
stub — the first release ships final-output match only (IMPLEMENTATION
§9.1 default). A future caller cannot accidentally use this file as a
cycle-accurate driver.

Group D placeholders (`tanh`, `exp`, `exp2`) currently raise
`NotImplementedError` on their lane_boxes (blocked on §6 HardFloat
round-trip strategy). To keep `gen_vectors.py` byte-identical with the
historical `gen_vpu_vectors.py`, those three ops route through a small
Python-`math` fallback that mirrors `gen_vpu_vectors.compute_expected`
exactly. When Group D lands, drop the fallback and route through the
real lane_box. Search for `_GROUP_D_FALLBACK` to find the seam.

`fp8pack` / `fp8unpack` are phased 2→1 / 1→2 — they don't fit into the
single-pulse `vpu_vectors.txt` format (see `gen_vpu_vectors.py:309-313`
and `IMPLEMENTATION_PLAN.md §4.6`), so `gen_vectors.py` skips them. The
functional entry point below still supports them: fp8pack takes two
BF16 rows via (`a_vec`, `b_vec`) and returns 16 UInt16 packed slots;
fp8unpack takes one packed row via `a_vec` and returns 32 BF16 bits
(low row followed by high row). The engine-layer E8M0 clamp from
`VectorEngine.scala:44-51` is applied here to match RTL.

`fp8` (the dead enum value between `csum` and `fp8pack` in `VPUOp`)
is unwired in `VectorEngine.scala` — see `IMPLEMENTATION_PLAN.md §9.7`.
`execute("fp8", ...)` raises `NotImplementedError` with a pointer at
the plan.
"""
...
from .lane_boxes.tanh_rec import TanhRec, TanhReq
...
_GROUP_D_FALLBACK = {"tanh", "exp", "exp2"}
...
class VectorEngineModel:
    ...
    def execute(
        ...
        Lane-count semantics:
          ...
          - Col reductions (csum/cmax/cmin): `a_vec` is `2 * num_lanes`
            long (two stacked rows), output is `2 * num_lanes` long with
            the scalar broadcast across every slot — matches the
            historical `gen_vpu_vectors.py` format.
          ...
          - `fp8`:       unwired dead enum (§9.7). Raises
            NotImplementedError.
        """
        n = self.p.num_lanes
        ...
        if op == "fp8":
            raise NotImplementedError(
                ...
                "fp8pack / fp8unpack instead. See IMPLEMENTATION_PLAN.md "
                "§9.7 for the history."
            )
    ...
    def cycle_step(self, *args, **kwargs):
        raise NotImplementedError(
            "Cycle-accurate driver is Phase 2; Phase 1f only needs execute()."
        )
    ...
    def _exec_col_reduce(self, op: str, a: list[int]) -> list[int]:
        """Reduce a 2-row × 16-col input to a single scalar broadcast 32 ways.

        This matches the `gen_vpu_vectors.py` format quirk where
        csum/cmax/cmin emit 32 expected slots, all populated with the
        same scalar (`sum`/`max`/`min` over every BF16 input).

        The funct model does the reduction via the real lane_boxes:
        ...
        Both paths upgrade from the historical FP64-via-`sum()` golden
        to bit-exact FP32 / sign-magnitude RTL semantics. A handful of
        rounding-edge cases will diverge from the committed
        `vpu_vectors_csum.txt` (see `IMPLEMENTATION_PLAN.md §4.3`); that
        is the intended outcome and the regen step rewrites the file.
        """
        n = self.p.num_lanes
        if len(a) != 2 * n:
            raise ValueError(
    ...
    def stream_col_reduce(
        ...
        BF16 accumulator. Mirrors §5's streaming loop for the Phase 3
        torch adapter, which needs true per-column output rather than
        the scalar-broadcast shape the `vpu_vectors.txt` file format
        requires.
...
# --------------------------------------------------------------------
#  Group D temporary fallback — drop when §6 HardFloat round-trip lands.
# --------------------------------------------------------------------

def _legacy_math_fallback(op: str, a_vec: list[int]) -> list[int]:
    """Mirror of `gen_vpu_vectors.py:compute_expected` for `tanh`, `exp`,
    `exp2`. We need byte-identity with the historical golden so the CLI
    smoke test passes; once the Group D lane_boxes are implemented, route
    through them and delete this helper."""
    out: list[int] = []
    for bits in a_vec:
...
def _bf16_bits_to_f32(bits: int) -> float:
    """Zero-pad BF16 → FP32 → Python float. Matches `bf16_hex_to_float`
    in `gen_vpu_vectors.py:59`."""
...
def _f32_to_bf16_bits_rne(x: float) -> int:
    """Mirror of `gen_vpu_vectors.py:float_to_bf16_hex` exactly. Special
    NaN/inf encoding plus RNE truncation. We inline this rather than
    using `bf16_utils.f32_to_bf16_bits_rne` so the byte-identity check
    passes — the IPT base helper has slightly different NaN payload
    handling that diverges on a handful of cases."""
    if math.isnan(x):
        return 0x7FC0
    if x == float("inf"):
        return 0x7F80
