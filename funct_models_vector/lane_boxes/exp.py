"""lane_boxes/exp.py — Group D placeholder for `ExpLane.scala` (`class Exp`).
...
RNE → BF16). Porting the full `RawFloat` ↔ Q(m,n) round-trip is the
topic of `IMPLEMENTATION_PLAN.md §6` ("transcendental Group D / §6 —
HardFloat round-trip strategy").

This skeleton exists so that:
  - tests can import the class and `pytest.skip("blocked on §6")`,
...
class Exp:
    ...
    def compute_now(self, req: FPEXReq) -> FPEXResp:
        raise NotImplementedError(
            "Exp / exp2 funct model is Group D — blocked on "
            "IMPLEMENTATION_PLAN.md §6 (HardFloat RawFloat ↔ Q(m,n) "
            "round-trip strategy)."
        )
    ...
    def step(self, op_name: str, req: Optional[FPEXReq]) -> Optional[FPEXResp]:
        ...
        if req is not None:
            raise NotImplementedError(
                "Exp.step with a live request is Group D — blocked on §6."
            )
