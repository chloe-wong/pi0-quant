...
Sources (under `dependencies/sp26-fp-units/.../vpuLUTs/`):
  ...
  - `ExLUT.scala:14-21`      → `gen_ex_lut`       (Group D; placeholder use)
  - `TanhLUT.scala:30-36`    → `gen_tanh_lut`     (Group D; placeholder use)
...
def gen_ex_lut(
    ...
    Used by the Group D Exp lane_box (deferred — placeholder consumer)."""
    return _gen_lut(addr_bits, m, n,
...
def gen_tanh_lut(
    ...
    `[0, 4)`. Used by the Group D Tanh lane_box (deferred — placeholder
    consumer)."""
