...
class InnerProductTreeParams:
    ...
    def intWidth(self) -> int:
        """Matches Scala: mulFmt.sigWidth + anchorHeadroom + extraBits  (when accumIntWidth == 0)"""
        if self.accumIntWidth > 0:
            return self.accumIntWidth
        return E4M3ProdFmt.sigWidth + self.anchorHeadroom + self.extraBits
